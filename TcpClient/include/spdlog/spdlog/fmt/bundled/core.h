...
/**
  ...
  subset of the API. ``joyinfmt::basic_string_view`` is used for format strings even
...
/**
  ...
  **Example**::
    ...
    std::string message = joyinfmt::format(my_string("The answer is {}"), 42);
  \endrst
 */
...
namespace internal {
void to_string_view(...);
using joyinfmt::v6::to_string_view;

// Specifies whether S is a string type convertible to joyinfmt::basic_string_view.
...
/**
  ...
  `~joyinfmt::basic_format_args` for passing into type-erased formatting functions
  such as `~joyinfmt::vformat`.
  \endrst
 */
...
/**
  \rst
  Constructs an `~joyinfmt::format_arg_store` object that contains references to
  arguments and can be implicitly converted to `~joyinfmt::format_args`. `Context`
  can be omitted in which case it defaults to `~joyinfmt::context`.
  See `~joyinfmt::arg` for lifetime considerations.
  \endrst
 */
...
template <typename Context> class basic_format_args {
 ...
 public:
  ...
  /**
   \rst
   Constructs a `basic_format_args` object from `~joyinfmt::format_arg_store`.
   \endrst
   */
...
/**
  ...
  **Example**::
    ...
    joyinfmt::print("Elapsed time: {s:.2f} seconds", joyinfmt::arg("s", 1.23));
  \endrst
 */
...
/**
  ...
  **Example**::
    ...
    #include <fmt/core.h>
    std::string message = joyinfmt::format("The answer is {}", 42);
  \endrst
*/
...
/**
  ...
  **Example**::
    ...
    joyinfmt::print(stderr, "Don't {}!", "panic");
  \endrst
 */
...
/**
  ...
  **Example**::
    ...
    joyinfmt::print("Elapsed time: {0:.2f} seconds", 1.23);
  \endrst
 */
template <typename S, typename... Args,
          FMT_ENABLE_IF(internal::is_string<S>::value)>
