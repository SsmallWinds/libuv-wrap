...
// logger->info("Some buffer {}", joyinlog::to_hex(v));
// char buf[128];
// logger->info("Some buffer {:X}", joyinlog::to_hex(std::begin(buf), std::end(buf)));
...
template<typename T>
struct formatter<joyinlog::details::bytes_range<T>>
{
    ...
    auto format(const joyinlog::details::bytes_range<Container> &the_range, FormatContext &ctx) -> decltype(ctx.out())
    {
    ...
    std::size_t put_newline(It inserter, std::size_t pos)
    {
        ...
        *inserter++ = '\n';
        ...
        if (put_positions)
        {
            joyinfmt::format_to(inserter, "{:<04X}: ", pos - 1);
            return 7;
        }
        else
        {
